...
module.exports = Backbone.Marionette.LayoutView.extend({
  ...
  initialize: function(options) {
    ...
    this.user = {
      'name': 'Nick',
      'tried-selection': 'all',
      'seasonal-selection': 'all',
      'beers-tried': [1,2],
      'currentBeerCollection': options.beers
    };
  },
  ...
  onBeerTriedSelected: function(event){
    ...
    case 'Tried it.':
        this.renderBeerListView(this.filterCollectionByTried(this.beers, true));
        break;
    case 'Need it.':
        this.renderBeerListView(this.filterCollectionByTried(this.beers, false));
        break;
    default:
  ...
  filterCollectionByTried: function(collection, shouldFilterByTried) {
    var self = this;
    var filteredCollection = new Backbone.Collection(collection.toJSON());
    filteredCollection.models = _.filter(collection.models, function(model) { 
      if (shouldFilterByTried) {
        return _.contains(self.user['beers-tried'], model.attributes.id);
      } else {
        return !_.contains(self.user['beers-tried'], model.attributes.id);
      }
    });
